    ...
    <div class="ty-control-group ty-product-options__item product-list-field clearfix">
        ...
        <div class="ty-pc-group__products" id="pc_{$po.group_id}">
            ...
            {elseif $po.configurator_group_type == "T"}
                {if $po.product}
                    <input type="hiddem" class="checkbox cm-no-change cm-configurator-disabled hidden" id="group_{$po.group_id}_product_{$po.product.product_id}" name="product_data[{$product.product_id}][configuration][{$po.group_id}][product_ids][]" value="{$po.product.product_id}" />
                    {if $show_price_values == true}<span class="price">{include file="common/price.tpl" value=$po.product.price}</span>{/if}
                    <div class="ty-pc-group__products-item-options">
