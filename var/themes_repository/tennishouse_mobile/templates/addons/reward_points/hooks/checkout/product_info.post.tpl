{*if !$cart.products.$key.extra.configuration}
    ...
    <div class="ty-reward-points__product-info">
        ...
        <span class="ty-control-group__item" id="price_in_points_{$key}">{$cart.products.$key.extra.points_info.display_price|fn_show_points}</span>
    </div>
    {/if}
    {if $cart.products.$key.extra.points_info.reward}
    <div class="ty-reward-points__product-info">
        <span class="ty-control-group__label">{__("reward_points")}:</span>
        <span class="ty-control-group__item" id="reward_points_{$key}">{$cart.products.$key.extra.points_info.reward|fn_show_points}</span>
    </div>
    {/if}
{/if*}