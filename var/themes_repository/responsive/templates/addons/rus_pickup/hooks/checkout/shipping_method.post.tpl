{if $cart.chosen_shipping.$group_key == $shipping.shipping_id && $shipping.module == 'pickup'}
    ...
    {if $display == "radio"}
        ...
        <div class="ty-checkout-select-store">
            {if $store_count > 1}
            <div class="ty-checkout-select-store__item-view">{include file="buttons/button.tpl" but_role="text" but_meta="cm-map-view-locations ty-btn__tertiary" but_text=__("view_all")}</div>
            {/if}
