...
{hook name="products:product_name"}
    {assign var="trunc_number" value=$trunc_number|default:"55"}
    {if $show_name}
        {if !$hide_links}<a href="{"products.view?product_id=`$product.product_id`{if $product.ohash}&`$product.ohash`{/if}"|fn_url}" class="product-title" {live_edit name="product:product:{$product.product_id}" phrase=$product.product}>{/if}{$product.product|truncate:$trunc_number:"...":true nofilter}{if !$hide_links}</a>{/if}
    {elseif $show_trunc_name}
        {if !$hide_links}<a href="{"products.view?product_id=`$product.product_id`{if $product.ohash}&`$product.ohash`{/if}"|fn_url}" class="product-title" title="{$product.product|strip_tags}" {live_edit name="product:product:{$product.product_id}" phrase=$product.product}>{/if}{$product.product|truncate:$trunc_number:"...":true nofilter}{if !$hide_links}</a>{/if}
    {/if}
{/hook}
{/capture}
{if $no_capture}
...
{if !($product.zero_price_action == "R" && $product.price == 0) && ($settings.General.inventory_tracking == "Y" && $settings.General.allow_negative_amount != "Y" && (($product_amount <= 0 || $product_amount < $product.min_qty) && $product.tracking != "ProductTracking::DO_NOT_TRACK"|enum) && $product.is_edp != "Y") && !($product.has_options && !$show_product_options)}
    {if $details_page && $product.out_of_stock_actions == "S"}
        ...
        {if !$auth.user_id }
        <div class="ty-control-group ty-input-append ty-product-notify-email {if $product.inventory_notification != "Y"}hidden{/if}" id="product_notify_{$obj_prefix}{$obj_id}">
    ...
    {/if}
{/if}
<!--follow_{$obj_prefix}{$obj_id}--></div>
{/if}
{/capture}
{if $no_capture}
    {assign var="capture_name" value="follow_`$obj_id`"}
    {$smarty.capture.$capture_name nofilter}
{/if}

{capture name="add_to_cart_`$obj_id`"}
{if $show_add_to_cart}
<div class="cm-reload-{$obj_prefix}{$obj_id} {$add_to_cart_class} ty-inline-block ty-add-to-cart-block" id="add_to_cart_update_{$obj_prefix}{$obj_id}">
                    ...
                    {if ($product_amount > 0 && $product_amount >= $product.min_qty) && $settings.General.inventory_tracking == "Y" || $details_page}
                        {if ($product_amount > 0 && $product_amount >= $product.min_qty) && $settings.General.inventory_tracking == "Y"}
                            ...
                            <div class="ty-control-group product-list-field ty-available-text">
                                ...
                                <span id="qty_in_stock_{$obj_prefix}{$obj_id}" class="ty-qty-in-stock ty-qty-left-in-stock ty-control-group__item">
                                    {__("left_in_stock")}&nbsp;{$product_amount}&nbsp;{__("items")}&nbsp;{include file="addons/development/common/tooltip.tpl" note_text=__("last_item_notification") icon_sign="!"}
                                </span>
                            </div>
                            {/if}
