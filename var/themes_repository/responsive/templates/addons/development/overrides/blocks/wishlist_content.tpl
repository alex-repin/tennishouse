<div class="top-wishlist ty-top-block__cell cm-hover-dropdown" id="wishlist_status_{$block.snapping_id}">
    {assign var="r_url" value=$config.current_url|escape:url}
    <div class="ty-top-block__cell-inner ty-dropdown-box__title ty-dropdown-box ty-top-block__wishlist-content cm-link {if $highlight}ty-top-block-change-state{/if}" data-href="{"wishlist.view"|fn_url}">
        <div class="ty-top-block__cell-content">
            <div class="ty-icon-wishlist">
                {if $smarty.session.wishlist.products|count}
                    <div class="ty-minicart__icon-counter">{$smarty.session.wishlist.products|count}</div>
                {/if}
            </div>
        </div>
    </div>
    <div class="cm-popup-box ty-dropdown-box__content cm-hover-dropdown-submenu" {if $is_open}style="display: block;"{/if}>
        <div class="ty-dropdown-box__content-inner">
            <div class="ty-dropdown-box__content-title">{__("wishlist")}</div>
                ...
                <div class="ty-cart-items">
                    {if $smarty.session.wishlist.products}
                        <ul class="ty-cart-items__list">
                            {foreach from=$smarty.session.wishlist.products key="key" item="p" name="wishlist_products"}
                                {if !$p.extra.parent}
                                        ...
                                        {if $block.properties.display_delete_icons == "Y"}
                                            ...
                                            <div class="ty-cart-items__list-item-tools cm-cart-item-delete">
                                                {if !$p.extra.exclude_from_calculate}
                                                    {include file="buttons/button.tpl" but_href="wishlist.unlike?pid=`$p.product_id`" but_meta="cm-ajax cm-ajax-full-render" but_target_id="wishlist_status*,add_to_wishlist*" but_role="delete" but_name="delete_cart_item"}
                                                {/if}
                                            </div>
                                            </div>
                                        {/if}
                    ...
                    {else}
                        <div class="ty-cart-items__empty">{__("wishlist_is_empty")}</div>
                    {/if}
                </div>
            </div>
            {if $smarty.session.wishlist.products && $block.properties.display_bottom_buttons == "Y"}
                <div class="ty-cart-content__buttons-wrapper">
                    <div class="ty-cart-content__buttons">
                        <div class="cm-cart-buttons ty-cart-content__buttons-container{if $smarty.session.wishlist.products} full-cart{else} hidden{/if}">
                            <div class="ty-float-right">
                                <a href="{"wishlist.view"|fn_url}" rel="nofollow" class="ty-btn ty-black-button">{__("view_wishlist")}</a>
                            </div>
                        </div>
                    </div>
                </div>
...
<!--wishlist_status_{$block.snapping_id}--></div>