...
<div class="ty-tabs cm-j-tabs{if $track} cm-track{/if} clearfix">
    ...
    {foreach from=$navigation.tabs item=tab key=key name=tabs}
        ...
        <li id="{$key}" class="ty-tabs__item{if $tab.js} cm-js{elseif $tab.ajax} cm-js cm-ajax{/if}{if $key == $active_tab} active{/if}"><a class="ty-tabs__a" {if $tab.href} href="{$tab.href|fn_url}"{/if}><h2>{$tab.title}</h2></a></li>
        {/if}
    {/foreach}
    </ul>
</div>
