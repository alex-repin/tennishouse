...
<div class="ty-step__container{if $edit}-active{/if} ty-step-two" data-ct-checkout="billing_shipping_address" id="step_two">
    ...
    <div id="step_two_body" class="ty-step__body{if $edit}-active{/if}{if !$edit} hidden{/if} cm-skip-save-fields">
            <form name="step_two_billing_address" class="{$ajax_form} cm-ajax-full-render cm-auto-submit cm-label-placeholder ty-mini-form-mode" data-auto-submit-dispatch="checkout.auto_save_user" action="{""|fn_url}" method="{if !$edit}get{else}post{/if}">
