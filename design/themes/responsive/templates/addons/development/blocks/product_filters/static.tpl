{** block-description:static_filter **}
...
{foreach from=$items item="filter" name="filters"}
    {if ($filter.slider || $filter.selected_ranges || $filter.ranges) && (!$category_data.tabs_categorization || $category_data.tabs_categorization != $filter.feature_id) && (!$category_data.subtabs_categorization || $category_data.subtabs_categorization != $filter.feature_id)}
        ...
        <div class="ty-product-filters__block">
            ...
            {if $filter.slider}
                {include file="addons/development/blocks/product_filters/components/product_filter_slider.tpl" filter_uid=$filter_uid id="slider_`$filter_uid`" filter=$filter ajax_div_ids=$ajax_div_ids dynamic=true filter_qstring=$filter_qstring reset_qstring=$reset_qstring allow_ajax=$allow_ajax}
            {else}
                {include file="addons/development/blocks/product_filters/components/product_filter_variants.tpl" filter_uid=$filter_uid filter=$filter ajax_div_ids=$ajax_div_ids collapse=$collapse filter_qstring=$filter_qstring reset_qstring=$reset_qstring allow_ajax=$allow_ajax}
            {/if}
        </div>
    {/if}
{/foreach}
