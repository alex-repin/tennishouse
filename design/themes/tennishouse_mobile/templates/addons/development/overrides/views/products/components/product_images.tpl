...
{if $product.image_pairs}
    ...
    {else}
        <div class="ty-product-thumbnails ty-center cm-image-gallery" id="images_preview_{$preview_id}">
            {strip}
                {if $image_pair_var}
                <a data-ca-gallery-large-id="det_img_link_{$preview_id}_{$image_id}" class="cm-thumbnails-mini active ty-product-thumbnails__item" style="width: {$th_size}px;height: {$th_size}px;">
                    {include file="common/image.tpl" images=$image_pair_var image_width=$th_size*2 image_height=$th_size*2 show_detailed_link=false obj_id="`$preview_id`_`$image_id`_mini"}
                </a>
                {/if}
                    ...
                    {foreach from=$product.image_pairs item="image_pair"}
                        {if $image_pair}
                                ...
                                {else}
                                    {assign var="img_id" value=$image_pair.image_id}
                                {/if}
                                <a data-ca-gallery-large-id="det_img_link_{$preview_id}_{$img_id}" class="cm-thumbnails-mini ty-product-thumbnails__item" style="width: {$th_size}px;height: {$th_size}px;">
                                {include file="common/image.tpl" images=$image_pair image_width=$th_size*3 image_height=$th_size*3 show_detailed_link=false obj_id="`$preview_id`_`$img_id`_mini"}
                                </a>
                        {/if}
                    {/foreach}
                {/if}
