...
<div>
    <form class="cm-ajax cm-ajax-force cm-ajax-full-render" name="coupon_code_form{$position}" action="{""|fn_url}" method="post">
        <input type="hidden" name="result_ids" value="checkout*,cart_status*,cart_items,payment-methods,catalog_promocode*" />
        ...
        {hook name="checkout:discount_coupons"}
            <div class="ty-discount-coupon__control-group ty-input-append">
                ...
                <input type="text" class="ty-input-text cm-hint" id="coupon_field{$position}" name="coupon_code" size="40" value="" placeholder="{__("promo_code")}"/>
    ...
    {capture name="promotion_info"}
        {hook name="checkout:applied_coupons_items"}
            ...
            <li class="ty-coupons__item">
                {__("coupon")} "{$coupon_code|strtoupper}"
                {assign var="_redirect_url" value=$config.current_url|escape:url}
                {assign var="coupon_code" value=$coupon_code|escape:url}
                {include file="buttons/button.tpl" but_href="checkout.delete_coupon?coupon_code=`$coupon_code`&redirect_url=`$_redirect_url`" but_role="delete" but_meta="ty-coupons__item-delete cm-ajax cm-ajax-full-render" but_target_id="checkout*,cart_status*,cart_items,catalog_promocode*"}
            </li>
            {/foreach}
