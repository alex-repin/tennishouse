        ...
        {if !$cart.shipping_failed && !empty($cart.chosen_shipping) && $cart.shipping_required}
            ...
            <ul id="tygh_shipping_method">
                {foreach from=$cart.chosen_shipping key="group_key" item="shipping_id"}
                    <li>
                        <div>{$product_groups[$group_key].shippings[$shipping_id].shipping}</div>
                        {if $cart.select_office.$group_key.$shipping_id && $product_groups[$group_key].shippings[$shipping_id].data.offices[$cart.select_office.$group_key.$shipping_id]}
                            {$product_groups[$group_key].shippings[$shipping_id].data.offices[$cart.select_office.$group_key.$shipping_id].City}, {$product_groups[$group_key].shippings[$shipping_id].data.offices[$cart.select_office.$group_key.$shipping_id].Address}{if $product_groups[$group_key].shippings[$shipping_id].data.offices[$cart.select_office.$group_key.$shipping_id].MetroStation} | м.  {['Ст.', 'ст.', 'м.', 'М.']|str_replace:'':$product_groups[$group_key].shippings[$shipping_id].data.offices[$cart.select_office.$group_key.$shipping_id].MetroStation}{/if}
                        {/if}
                    </li>
                {/foreach}
            </ul>
