...
{if $view_mode != "list"}
    ...
    {if $display == "checkbox"}
        {assign var="_but_text" value=__("add_players")}
    ...
    {/if}
        {include file="buttons/button.tpl" but_id="opener_picker_`$data_id`" but_href="players.picker?display=`$display`&extra=`$extra_var`&picker_for=`$picker_for`&data_id=`$data_id`&shared_force=`$shared_force`"|fn_url but_text=$_but_text but_role="add" but_target_id="content_`$data_id`" but_meta="cm-dialog-opener `$but_meta`" but_icon=$but_icon}
    {if $placement == 'right'}
        </div></div>
    {/if}
...
{if $view_mode != "button"}
    {if $display != "radio"}
        ...
        <thead>
        <tr>
            <th width="100%">{__("player_name")}</th>
            <th>&nbsp;</th>
        </tr>
        </thead>
        <tbody id="{$data_id}"{if !$item_ids} class="hidden"{/if}>
        {include file="addons/development/pickers/players/js.tpl" player_id="`$ldelim`player_id`$rdelim`" player_name="`$ldelim`player_name`$rdelim`" holder=$data_id clone=true}
        {if $item_ids}
        {foreach from=$item_ids item="player" name="items"}
            {assign var="player_info" value=$player|fn_get_player_data}
            {include file="addons/development/pickers/players/js.tpl" player_id=$player player_name="`$player_info.player`" holder=$data_id first_item=$smarty.foreach.items.first}
        {/foreach}
        {/if}
        </tbody>
