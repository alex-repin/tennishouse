...
<div class="cm-popup-box dropleft {if !$hide_for_vendor}dropdown{/if} {$popup_additional_class}">
    ...
    {if $id && !$hide_for_vendor}
        ...
        {if $table && $object_id_name}{capture name="_extra"}&table={$table}&id_name={$object_id_name}{/capture}{/if}
            ...
            {/if}
            {if $notify_sms}
                <li><a><label for="{$prefix}_{$id}_notify_sms">
                    <input type="checkbox" name="__notify_user_sms" id="{$prefix}_{$id}_notify_sms" value="Y" checked="checked" onclick="Tygh.$('input[name=__notify_user_sms]').prop('checked', this.checked);" />
                    {$notify_text|default:__("notify_customer_sms")}</label></a>
                </li>
            {/if}
