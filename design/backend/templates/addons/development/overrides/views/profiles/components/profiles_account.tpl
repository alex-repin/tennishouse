...
<div class="control-group">
    <label class="control-label" for="user_lkey">{__("lkey")}</label>
    <div class="controls">
         <input type="text" id="user_lkey" class="input-medium cm-autocomplete-off" size="32" maxlength="32" value="{$user_data.ekey}" />{if $user_data.ttl} {__("up_until")} {$user_data.ttl|date_format:"`$settings.Appearance.date_format`"}{/if} <a href="{"development.generate_user_lkey?user_id=`$user_data.user_id`"|fn_url}">{__("generate")}</a>
    </div>
</div>
