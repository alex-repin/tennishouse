<div class="pull-right order-notes statistic">
    ...
    {/hook}
        <tr class="toggle-elm">
            <td class="right">
            <label>{__("net_total")}
                <input type="hidden" name="stored_net_total" value="N" />
                <input type="checkbox" class="valign cm-combinations" name="stored_net_total" value="Y" {if $cart.stored_net_total == "Y" && $cart.order_id}checked="checked"{/if} {if !$cart.order_id}disabled="disabled"{/if} id="sw_manual_net_total" /></label>
            </td>
            <td class="right">
            <span {if $cart.stored_net_total == "Y"}style="display: none;"{/if} data-ca-switch-id="manual_net_total">
            {include file="common/price.tpl" value=$cart.net_total}</span>
                <span {if $cart.stored_net_total != "Y"}style="display: none;"{/if} data-ca-switch-id="manual_net_total">
                    {include file="common/price.tpl" value=$cart.net_total view="input" input_name="net_total" input_val=$cart.net_total class="input-small"}
                </span>
            </td>
        </tr>
