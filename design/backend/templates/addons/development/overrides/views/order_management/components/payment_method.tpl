    ...
    {if $settings.General.min_order_amount <= $cart.total}
        ...
        <div class="control-group">
            ...
            <select name="payment_id" id="payment_methods" onchange="Tygh.$.selectPaymentMethod(Tygh.$(this).val())">
                <option value=""> - {__("select_payment")} - </option>
                {foreach from=$payment_methods item="pm" name="pay"}
                <option value="{$pm.payment_id}" {if $cart.payment_id == $pm.payment_id}{assign var="selected_payment_id" value=$pm.payment_id}selected="selected"{/if}>{$pm.payment}</option>
                {/foreach}
            </select>
            </div>
        </div>
