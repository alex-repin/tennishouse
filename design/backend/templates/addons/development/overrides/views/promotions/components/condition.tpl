...
<div class="option clearfix">
    ...
    {elseif $schema.conditions[$condition_data.condition].type == "city_textbox"}
        <div class="ty-city-textbox">
            <div class="ty-city-textbox-section">
                <textarea name="{$prefix}[value]" id="mixed_select_{$p_md}" rows="8" class="input-full">{$condition_data.value}</textarea>
            </div>
            <div class="ty-city-textbox-note">{__("text_ip_cities_cities_wildcards")}</div>
        </div>
    {elseif $schema.conditions[$condition_data.condition].type == "state_selectbox"}
        {$states = $schema.conditions[$condition_data.condition].variants|default:$schema.conditions[$condition_data.condition].variants_function|fn_get_promotion_variants}
        {include file="common/double_selectboxes.tpl"
            first_name="`$prefix`[value]"
            first_data=$condition_data.value|fn_get_state_names:$states
