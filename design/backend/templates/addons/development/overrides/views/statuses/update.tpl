...
<fieldset>
    ...
    <div class="control-group">
        <label for="sms_text_{$id}" class="control-label">{__("sms_text")}:</label>
        <div class="controls cm-no-hide-input">
            <textarea id="sms_text_{$id}" name="status_data[sms_text]" class="input-textarea-long" {if $disable_input}disabled="disabled"{/if}>{$status_data.sms_text}</textarea>
            {if "ULTIMATE"|fn_allowed_for}
            {include file="buttons/update_for_all.tpl" display=$show_update_for_all object_id="`$id`_sms_text" name="update_all_vendors[sms_text]" hide_element="sms_text_`$id`"}
            {/if}
        </div>
    </div>
