...
<tbody>
{foreach from=$category_data.prices item="price" key="_key" name="prod_prices"}
<tr class="cm-row-item">
    <td width="5%" class="{$no_hide_input_if_shared_product}">
        {if $price.lower_limit == "1" && $price.usergroup_id == "0"}
            &nbsp;{$price.lower_limit}
        {else}
        <input type="text" name="category_data[prices][{$_key}][lower_limit]" value="{$price.lower_limit}" class="input-micro" />
        {/if}</td>
    <td width="20%" class="{$no_hide_input_if_shared_product}">
        ...
        {else}
        <input type="text" name="category_data[prices][{$_key}][price]" value="{if $price.percentage_discount == 0}{$price.price|default:"0.00"}{else}{$price.percentage_discount}{/if}" size="10" class="input-medium" />
        {/if}</td>
    <td width="25%" class="{$no_hide_input_if_shared_product}">
        {if $price.lower_limit == "1" && $price.usergroup_id == "0"}
            &nbsp;{if $price.percentage_discount == 0}{__("absolute")}{else}{__("percent")}{/if}
        {else}
        <select class="span3" name="category_data[prices][{$_key}][type]">
            <option value="P" {if $price.type == 'P'}selected="selected"{/if}>{__("percent")} (%)</option>
        </select>
        {/if}</td>
    {if !"ULTIMATE:FREE"|fn_allowed_for}
    <td width="25%" class="{$no_hide_input_if_shared_product}">
        {if $price.lower_limit == "1" && $price.usergroup_id == "0"}
            &nbsp;{__("all")}
        {else}
        <select id="usergroup_id" name="category_data[prices][{$_key}][usergroup_id]" class="span3">
...
<tr class="{cycle values="table-row , " reset=1}{$no_hide_input_if_shared_product}" id="box_add_qty_discount">
    <td width="5%">
        <input type="text" name="category_data[prices][{$new_key}][lower_limit]" value="" class="input-micro" /></td>
    <td width="20%">
        <input type="text" name="category_data[prices][{$new_key}][price]" value="0.00" size="10" class="input-medium" /></td>
    <td width="25%">
    <select class="span3" name="category_data[prices][{$new_key}][type]">
        <option value="P">{__("percent")} (%)</option>
    </select></td>
    {if !"ULTIMATE:FREE"|fn_allowed_for}
    <td width="25%">
        <select id="usergroup_id" name="category_data[prices][{$new_key}][usergroup_id]" class="span3">
            {foreach from=fn_get_default_usergroups() item="usergroup"}
                <option value="{$usergroup.usergroup_id}">{$usergroup.usergroup}</option>
            {/foreach}
            {foreach from=$usergroups item="usergroup"}
