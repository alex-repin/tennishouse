...
{foreach from=$cp.product_configurator_groups item="pg" key="group_id"}
    {if $pg.selected_id}
        ...
        <tr class="cm-ex-op row-more row-gray">
            <td class="left">&nbsp;</td>
            <td>
                {include file="addons/product_configurator/views/products/components/product_configuration.tpl" key_conf=$_key product=$cp po=$pg}
            </td>
            <td width="3%">
    ...
    {else}
        <tr class="cm-ex-op row-more row-gray">
            <td class="left">&nbsp;</td>
            <td colspan="{$colspan}">
                {include file="addons/product_configurator/views/products/components/product_configuration.tpl" product=$cp po=$pg}
            </td>
        </tr>
    {/if}
{/foreach}