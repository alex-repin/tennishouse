...
<div class="ty-control-group ty-product-options__item product-list-field clearfix">
    ...
    <div class="ty-pc-group__products" id="pc_{$po.group_id}">
        ...
        <input type="hidden" name="cart_products[{$key}][configuration][{$po.group_id}][is_separate]" value="{$cp.extra.configuration.$group_id.is_separate}" />
        {if $po.configurator_group_type == "S"}
            <div class="ty-pc-group__products-item-block">
                <select name="cart_products[{$key}][configuration][{$po.group_id}][product_ids][]" id="group_{$po.group_id}" class="cm-dropdown cm-options-update" data-cesbClass="ty-sb-popup-large" {if $product.configuration_out_of_stock}disabled="disabled"{/if} onchange="$('#recalculate_cart').click();">
            ...
            <div class="ty-pc-group__products-item-options">
                {foreach from=$po.products item="group_product" name="descr_links"}
                    {if $group_product.selected == "Y" && $group_product.product_options}
                        <input type="hidden" name="product_data[{$group_product.product_id}][item_id]" value="{$key}" />
                        <input type="hidden" name="product_data[{$group_product.product_id}][group_id]" value="{$po.group_id}" />
                        {include file="views/products/components/select_product_options.tpl" product=$group_product id=$group_product.product_id product_options=$group_product.product_options name="product_data" request_obj_prefix=$obj_prefix request_obj_id=$cp.product_id}
                    {/if}
                {/foreach}
            </div>
            {if $po.show_amount}
                <div class="ty-pc-group__products-item-amount">
                    <div class="ty-qty clearfix{if $settings.Appearance.quantity_changer == "Y"} changer{/if}">
                        ...
                        <div class="ty-center ty-value-changer cm-value-changer">
                            ...
                            {/if}
                            <input type="text" size="5" class="ty-value-changer__input cm-amount" id="pc_qty_{$po.group_id}" name="cart_products[{$key}][configuration][{$po.group_id}][amount]" value="{$po.amount}" {if $product.qty_step > 1} data-ca-step="{$product.qty_step}"{/if} data-ca-min-qty="1" data-ca-max-qty="{$po.max_amount}" />
                            {if $settings.Appearance.quantity_changer == "Y"}
        ...
        {elseif $po.configurator_group_type == "R" }
            ...
            <tbody id="group_{$po.group_id}">
                ...
                <tr>
                    <td><input  id="group_{$po.group_id}_product_0" type="radio" class="radio {if $disable_product}cm-configurator-disabled{/if}" name="cart_products[{$key}][configuration][{$po.group_id}][product_ids][]" value="0" checked="checked" {if $group_product.disabled || $disable_product}disabled="disabled"{/if} /></td>
                    <td>&nbsp;{__("none")}</td>
                    <td>&nbsp;</td>
                </tr>
                {/if}
                ...
                <tr>
                    <td><input type="radio" class="radio cm-no-change {if $disable_product}cm-configurator-disabled{/if}" id="group_{$po.group_id}_product_{$group_product.product_id}" name="cart_products[{$key}][configuration][{$po.group_id}][product_ids][]" value="{$group_product.product_id}" {if $group_product.selected == "Y" && false == $disable_product}checked="checked"{/if} {if $group_product.disabled == true || $disable_product}disabled="disabled"{/if} /></td>
        ...
        {elseif $po.configurator_group_type == "C"}
            {if $po.products}
                ...
                <tr>
                    <td>
                        <input type="checkbox" class="checkbox cm-no-change {if $disable_product}cm-configurator-disabled{/if}" id="group_{$po.group_id}_product_{$group_product.product_id}" name="cart_products[{$key}][configuration][{$po.group_id}][product_ids][]" value="{$group_product.product_id}" {if $group_product.selected == "Y" && false == $disable_product}checked="checked"{/if} {if $group_product.disabled == true || $disable_product}disabled="disabled"{/if} /></td>
                    <td style="width: 100%">{if $group_product.is_accessible}{include file="common/popupbox.tpl" id="description_`$po.group_id`_`$group_product.product_id`" link_text=$group_product.product text=$group_product.product href="products.configuration_product?group_id=`$po.group_id`&product_id=`$group_product.product_id`" content=""}{else}{$group_product.product}{/if}{if $group_product.recommended == "Y"} <span>({__("recommended")})</span>{/if}</td>
                    <td class="right">&nbsp;{if $show_price_values == true}<span class="price">{include file="common/price.tpl" value=$group_product.price}</span>{/if}</td>
                </tr>
                {/foreach}
        ...
        {elseif $po.configurator_group_type == "T"}
            {if $po.product}
                <div class="ty-pc-group__products-item-block">
                <input type="checkbox" class="checkbox cm-no-change cm-configurator-disabled hidden" id="group_{$po.group_id}_product_{$po.product.product_id}" name="cart_products[{$key}][configuration][{$po.group_id}][product_ids][]" value="{$po.product.product_id}" checked="checked" />
                </div>
                <div class="ty-pc-group__products-item-options">
                    {if $po.product.product_options}
                        {include file="addons/product_configurator/views/products/components/configuration_product_options.tpl" product=$po.product id=$po.product.product_id product_options=$po.product.product_options name="cart_products[`$key`][configuration][`$po.group_id`][options]" request_obj_prefix=$obj_prefix request_obj_id=$cp.product_id}
                    {/if}
                </div>
            {/if}
        {/if}
