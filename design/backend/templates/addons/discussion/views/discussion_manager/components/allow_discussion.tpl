    ...
    <div class="controls">
        ...
        {if "discussion.add"|fn_check_view_permissions}
            <select name="{$prefix}[discussion_type]" id="discussion_type">
                <option {if $discussion.type == "B" || (!$discussion && $object_type == 'P')}selected="selected"{/if} value="B">{__("communication")} {__("and")} {__("rating")}</option>
                ...
                <option {if $discussion.type == "D" || (!$discussion && $object_type != 'P')}selected="selected"{/if} value="D">{__("disabled")}</option>
            </select>
        {else}
