...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    if ($mode == 'notify') {
        ...
        $signature = strtoupper(md5($processor_data['processor_params']['av_sign']) . md5($data['shop_id'] . $order_id . ($order_info['total'] * 100)));
...
function fn_avangard_registration($number_order, $order_info, $processor_data)
{
    ...
    $order->appendChild($dom->createElement('amount', $order_info['total'] * 100));
    ...
    $order->appendChild($dom->createElement('client_phone', $order_info['b_phone']));
