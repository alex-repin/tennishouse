...
if ($mode == 'autocomplete_city') {
    ...
    if (defined('AJAX_REQUEST') && $params['q']) {
        ...
        if (preg_match('/^[a-zA-Z]+$/',$params['q'])) {
            if ((Registry::get('addons.rus_cities.status') == 'A')) {
                ...
                if (!empty($city)) {
                    ...
                    fn_set_notification('E', __('notice'), __('shippings.spsr.lang_error'));
                    exit();
                }
            }
        }

        $join = db_quote("LEFT JOIN ?:rus_spsr_cities as c ON c.city_id = d.city_id");
        ...
        $cities = db_get_array("SELECT d.city, c.city_code FROM ?:rus_spsr_city_descriptions as d ?p WHERE city LIKE ?l AND lang_code = ?s  ?p  LIMIT ?i", $join , $search , 'ru', $condition, 10);
...
} elseif ($mode == 'spsr_get_city_data') {
    $params = $_REQUEST;
    ...
    if (defined('AJAX_REQUEST')) {
        ...
        $data = SpsrFrame::SpsrCityId($location);

        $city_data = array(
            'from_city_id' => $data,
        );

        Registry::get('view')->assign('spsr_new_city_data', $city_data);
        Registry::get('view')->display('addons/rus_spsr/views/shippings/components/services/spsr.tpl');
        exit;
    }
}
