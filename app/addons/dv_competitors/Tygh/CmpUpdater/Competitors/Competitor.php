...
class Competitor
{
    ...
    public function domParse($content)
    {
        $result = array();
        $dom = new \DOMDocument();
    ...
    public function regParse($content)
    {
        $result = array();
    ...
    {
        $tmp = microtime();
        // $result = Http::get($this->competitor['link']);
        $result = Http::get("https://www.saletennis.com/catalog/product/tiennisnaia-rakietka-babolat-pure-drive-2021-10488/");
        fn_print_r(microtime());
        $links = $this->domParse($result);
        // $links = $this->regParse($result);
        fn_print_r(microtime());
        fn_print_die($links);
        fn_print_r(microtime(false));

        fn_print_die(microtime(false) - $tmp);
        ...
        if (!empty($this->feed)) {
            ...
            if (!empty($this->parsed_data['products'])) {
                ...
                $warehouse_inventories = db_get_hash_multi_array("SELECT ?:product_warehouses_inventory.*, ?:product_options_inventory.combination FROM ?:product_warehouses_inventory LEFT JOIN ?:product_options_inventory ON ?:product_options_inventory.combination_hash = ?:product_warehouses_inventory.combination_hash WHERE ?:product_warehouses_inventory.warehouse_id = ?i AND ?:product_warehouses_inventory.product_id IN (?n)", array('product_id', 'combination_hash'), $this->competitor, array_keys($product_data));

                $other_inventories = db_get_hash_multi_array("SELECT SUM(amount) AS amount, combination_hash, product_id FROM ?:product_warehouses_inventory WHERE combination_hash != '0' AND warehouse_id != ?i AND product_id IN (?n) GROUP BY combination_hash", array('product_id', 'combination_hash'), $this->competitor, array_keys($product_data));
                ...
                if (!empty($out_of_stock)) {
                    db_query("UPDATE ?:product_warehouses_inventory SET amount = '0' WHERE product_id IN (?n) AND warehouse_id = ?i", $out_of_stock, $this->competitor);
                ...
                if (!empty($updated_by_combinations)) {
                    ...
                    foreach ($updated_by_combinations as $pr_id => $combs) {
                        ...
                        if (!empty($out)) {
                            db_query("UPDATE ?:product_warehouses_inventory SET amount = '0' WHERE combination_hash IN (?n) AND warehouse_id = ?i", $out, $this->competitor);
    ...
    private function createProduct($product)
    {
        ...
        if (!empty($product_id)) {
            ...
            if (count($product['combinations']) == 1) {
                $_data[] = array(
                    'warehouse_hash' => fn_generate_cart_id($product_id, array('warehouse_id' => $this->competitor)),
                    'warehouse_id' => $this->competitor,
    ...
    protected function getSyncSchema()
    {
        ...
        return $schema[$this->competitor] ?? array();
    }
}
