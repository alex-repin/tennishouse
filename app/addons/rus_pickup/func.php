...
function fn_rus_pickup_install()
{
    ...
    $pickup_id = db_query("INSERT INTO ?:settings_objects (`edition_type`, `name`, `section_id`, `section_tab_id`, `type`, `value`, `position`, `is_global`) VALUES ('ROOT', 'pickup_enabled', '7', '0', 'C', 'Y', '190', 'Y')");

    $languages = Languages::getAll();

    foreach ($languages as $lang_code => $lang_data) {

        $lang_var = Values::getLangVar('active', $lang_code);

        if ($lang_code == 'ru') {
            $setting_name = $lang_var . ' "Самовывоз"';
        } else {
            $setting_name = $lang_var . ' "Pickup"';
        }

        db_query("INSERT INTO ?:settings_descriptions (`object_id`, `object_type`, `lang_code`, `value`, `tooltip`) VALUES (?i, 'O', ?s, ?s, '')", $pickup_id, $lang_code, $setting_name);
    }
...
{
    $pickup_id = db_get_field('SELECT object_id FROM ?:settings_objects WHERE name = ?s', 'pickup_enabled');
    if (!empty($pickup_id)) {
        db_query('DELETE FROM ?:settings_objects WHERE object_id = ?i', $pickup_id);
        db_query('DELETE FROM ?:settings_descriptions WHERE object_id = ?i', $pickup_id);
    }
